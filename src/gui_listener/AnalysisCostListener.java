...
import service.AnalysisCostService;
import util.JFreeChartCreateUtil;

public class AnalysisCostListener implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		AnalysisCostService analysisCostService = new AnalysisCostService();
		JButton b = (JButton) e.getSource();
		if( b == analysisCostPanel.getChooseYear() ) {
			...
			if(input == null) //用户选择取消
				return ;
			if(input.length() == 0) {
				JOptionPane.showMessageDialog(analysisCostPanel, "年份不能为空");
			}
			else if(pattern.matcher(input).matches()) { //判断用户输入的年份是否为纯数字
				year = Integer.parseInt(input);
			}
			else {
				JOptionPane.showMessageDialog(analysisCostPanel, "年份不能存在除数字外的其他字符");
			}

		}
		if( b == analysisCostPanel.getChooseMonth() ) {
		...
		if( b == analysisCostPanel.getRefreshButton() ) {

			try {
				if( year == 0 )
				{
	                JOptionPane.showMessageDialog(analysisCostPanel, "请先选择年份!");
	                return;
				}
				if( month == 0 )
				{
	                JOptionPane.showMessageDialog(analysisCostPanel, "请先选择月份!");
	                return;
				}
				List<CostRecord> dataList = analysisCostService.monthCostRecord(year, month);
				JFreeChart chart = JFreeChartCreateUtil.getCostChartCreate(dataList);
				analysisCostPanel.getCostChartPanel().setChart(chart);
				analysisCostPanel.getCostChartPanel().updateUI();

			} catch (ParseException e1) {
				e1.printStackTrace();
			}
		}
		if( b == analysisCostPanel.getReturnButton() ) {
